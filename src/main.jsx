...
function VoiceScreen({ onBack, onSend, onLiveTurn }) {
  ...
  const beginLive = async session => {
    ...
    const socket = new WebSocket(`${session.gateway_url}${session.gateway_url.includes('?') ? '&' : '?'}access_token=${encodeURIComponent(session.token)}`); liveSocketRef.current = socket
    const startCapture = () => {
