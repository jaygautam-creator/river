...
function PrivacyPanel({ user, enabled, onToggle, onExport, onClose, onAccountDeleted }) {
...
function MemoryPanel({ storylines, proposals = [], onUpdate, onDelete, onApprove, onReject, onClose }) {
...
function App({ user, onLogout }) {
  ...
  useEffect(() => {
    const boot = async () => {
      try { const [available] = await Promise.all([refreshThreads(), refreshReminders(), api('/api/privacy/preferences').then(data => setMemoryEnabled(data.memory_enabled))]); if (available[0]) await loadThread(available[0].id) } catch (err) { setError(err.message) }
    }
    boot()
  }, [])
  useEffect(() => {
