...
import { ArrowUp, Bell, BookOpen, Check, ChevronDown, CircleHelp, Clock3, Compass, Copy, Ellipsis, Headphones, Loader2, LogOut, Menu, Mic, MoreHorizontal, Pencil, Plus, Quote, Search, Send, Settings2, Trash2, X, Zap } from 'lucide-react'
import './styles.css'
...
function RiverMark({ className = '' }) {
  return <img className={`river-mark ${className}`.trim()} src="/river-mark.svg" alt="" aria-hidden="true" />
}
...
function Auth({ onAuth }) {
  ...
  const submit = async e => {
    ...
    try {
      if (mode === 'recovery') { await api('/api/auth/password-reset/recover', { method: 'POST', body: JSON.stringify({ email, code: recoveryCode, password }) }); setMode('login'); setPassword(''); setRecoveryCode(''); setNotice('Your password was reset. Sign in with your new password.'); return }
      if (mode === 'reset') {
      ...
      if (mode === 'signup' && Array.isArray(data.recovery_codes) && data.recovery_codes.length) { setPendingUser(data.user); setPendingCodes(data.recovery_codes); return }
      onAuth(data.user)
    } catch (err) { setError(err.message) } finally { setBusy(false) }
  }
  const copyCodes = () => { navigator.clipboard?.writeText(pendingCodes.join('\n')); setCopied(true); setTimeout(() => setCopied(false), 2000) }
  if (pendingCodes) return <main className="auth-page">
    <div className="auth-glow glow-a" /><div className="auth-glow glow-b" />
    <div className="auth-brand"><RiverMark className="brand-mark" /><span>river</span></div>
    <section className="auth-card">
      <div className="auth-intro"><div className="eyebrow"><span className="eyebrow-dot" /> save these now</div><h1>Your recovery codes</h1><p>There is no email recovery yet, so these are the <strong>only</strong> way back into your account if you forget your password. Save them somewhere safe. Each code works once.</p></div>
      <div className="recovery-codes">{pendingCodes.map(code => <code key={code}>{code}</code>)}</div>
      <button type="button" className="ghost-button" onClick={copyCodes}><Copy size={14} /> {copied ? 'Copied' : 'Copy codes'}</button>
      <button className="primary-button auth-submit" onClick={() => onAuth(pendingUser)}>I’ve saved them — continue<ArrowUp size={17} /></button>
    </section>
  ...
  return <main className="auth-page">
    ...
    <section className="auth-card">
      ...
      <form onSubmit={submit} className="auth-form">
        ...
        {mode === 'recovery' && <label>Recovery code<input value={recoveryCode} onChange={e => setRecoveryCode(e.target.value)} placeholder="xxxxx-xxxxx" autoComplete="off" spellCheck={false} required /></label>}
        <label>{mode === 'reset' || mode === 'recovery' ? 'New password' : 'Password'}<input type="password" minLength={mode === 'reset' ? 12 : mode === 'recovery' ? 8 : 6} value={password} onChange={e => setPassword(e.target.value)} placeholder={mode === 'reset' ? 'At least 12 characters' : mode === 'recovery' ? 'At least 8 characters' : 'At least 6 characters'} required /></label>
        ...
        <button className="primary-button auth-submit" disabled={busy}>{busy ? <Loader2 className="spin" size={17} /> : mode === 'signup' ? 'Begin your thread' : mode === 'login' ? 'Welcome back' : mode === 'recovery' ? 'Reset password' : 'Save new password'}<ArrowUp size={17} /></button>
      </form>
      {mode === 'login' && <button type="button" className="auth-link" onClick={() => { setMode('recovery'); setError(''); setNotice('') }}>Forgot your password? Use a recovery code</button>}
...
function Sidebar({ user, threads, activeThreadId, onSelectThread, onNew, onLogout, onReviewMemory, onPrivacy, onToday, onMemory, onRenameThread, onDeleteThread }) {
  ...
  return <aside className="sidebar">
    <div className="sidebar-top">
      ...
      {moreOpen && <div ref={moreMenuRef} className="sidebar-more-menu" role="menu" aria-label="River options">
        ...
        <button type="button" role="menuitem" onClick={() => runMoreAction(onReviewMemory)}><Zap size={15} /> Review memory</button>
    ...
    <div className="sidebar-bottom">
      ...
      <button className="seed-button" onClick={onReviewMemory}><Zap size={15} /> Review your memory</button>
...
function PrivacyPanel({ user, enabled, memoryMode, retentionDays, onToggle, onMemoryModeChange, onRetentionChange, onExport, onClose, onAccountDeleted }) {
  ...
  const loadSecurity = async () => { try { const [sessionData, eventData, passkeyData, recoveryData] = await Promise.all([api('/api/auth/sessions'), api('/api/auth/security-events'), api('/api/auth/passkeys/status'), api('/api/auth/recovery-codes/status').catch(() => null)]); setSessions(sessionData.sessions.filter(session => !session.revoked_at)); setSecurityEvents(eventData.events || []); setPasskeyStatus(passkeyData); if (recoveryData) setRecoveryRemaining(recoveryData.remaining) } catch {} }
  const regenerateRecovery = async () => { if (!window.confirm('Generate a new set of recovery codes? Your old codes will stop working.')) return; setBusy(true); try { const result = await api('/api/auth/recovery-codes/regenerate', { method: 'POST', body: '{}' }); setRecoveryCodes(result.recovery_codes); setRecoveryRemaining(result.recovery_codes.length); setNotice('New recovery codes generated. Save them now — they are shown only once.') } catch (err) { setNotice(err.message) } finally { setBusy(false) } }
...
function MemoryPanel({ storylines, proposals = [], view = 'all', onUpdate, onDelete, onApprove, onReject, onHistory, onClose }) {
  ...
  if (view === 'review') return <aside className="memory-panel" aria-label="Review memory"><div className="panel-head"><div><div className="eyebrow"><span className="eyebrow-dot" /> review before remembering</div><h2>Review memory</h2></div><button className="icon-button" aria-label="Close memory review" onClick={onClose}><X size={18} /></button></div><p className="panel-intro">River brings uncertain or sensitive details here for your decision. Nothing in this queue becomes a memory unless you approve it.</p><div className="proposal-list review-only">{proposals.length ? proposals.map(p => <article className="proposal-card" key={p.id}><strong>{p.topic}</strong><p>{p.summary}</p><small>{p.sensitivity === 'sensitive' ? 'Sensitive memory · ' : ''}{p.conflict_storyline_id ? 'May revise an existing memory · ' : ''}{Math.round(p.confidence * 100)}% confidence · “{p.source_quote}”</small><div className="edit-actions"><button className="ghost-button" onClick={() => onReject(p.id)}>Not now</button><button className="save-button" onClick={() => onApprove(p.id)}><Check size={14} /> Remember</button></div></article>) : <div className="empty-memory review-empty"><div className="empty-icon"><Check size={20} /></div><strong>Nothing is waiting for review.</strong><span>River will bring uncertain or sensitive details here when it needs your approval.</span></div>}</div><div className="panel-footer"><CircleHelp size={14} /><span>Open memory lets you browse and edit what you have already approved.</span></div></aside>
...
function EmptyState({ user, onMemory, onPrompt }) {
  return <div className="empty-state"><div className="hello-orbit"><div className="orbit orbit-one" /><div className="orbit orbit-two" /><RiverMark className="hero-river-mark" /></div><div className="eyebrow centered"><span className="eyebrow-dot" /> your space, your pace</div><h1>What’s here today,<br /><em>{user.name.split(' ')[0]}?</em></h1><p>Start anywhere. River keeps your conversations private and gradually learns the clear things that matter; sensitive or uncertain memories stay under your review.</p><ol className="onboarding-steps" aria-label="How River works"><li><strong>Talk in threads</strong><span>Keep different parts of life in their own space.</span></li><li><strong>Memory with care</strong><span>Clear everyday details can be remembered; sensitive details ask first.</span></li><li><strong>Come back anytime</strong><span>Search, edit, export, or delete on your terms.</span></li></ol><div className="prompt-grid"><button onClick={() => onPrompt('I have something on my mind that I want to unpack.')}><span>Unpack something</span><small>that’s been circling</small><ArrowUp size={15} /></button><button onClick={() => onPrompt('Help me make a small plan for today.')}><span>Make a small plan</span><small>one next step at a time</small><ArrowUp size={15} /></button><button className="memory-prompt" onClick={onMemory}><span>Explore memory</span><small>see what River has learned</small><BookOpen size={15} /></button></div></div>
}
...
function App({ user, onLogout }) {
  ...
  const [memoryView, setMemoryView] = useState('all')
  const [busy, setBusy] = useState(false)
  ...
  const openMemory = () => { setMemoryView('all'); setMemoryOpen(true) }
  const reviewMemory = () => { setMemoryView('review'); setMemoryOpen(true) }
  ...
  return <><div className="app-shell"><Sidebar user={user} threads={threads} activeThreadId={activeThreadId} onSelectThread={loadThread} onLogout={onLogout} onNew={newThread} onReviewMemory={reviewMemory} onPrivacy={() => setPrivacyOpen(true)} onToday={() => setTodayOpen(true)} onMemory={openMemory} onRenameThread={renameThread} onDeleteThread={deleteThread} /><main className="main-column"><header className="topbar"><div className="mobile-brand"><RiverMark className="brand-mark small" />river</div><div className="session-label"><span className="live-dot" /> ongoing thread <ChevronDown size={14} /></div><div className="top-actions"><button className="icon-button" aria-label="Search" onClick={() => setSearchOpen(true)}><Search size={17} /></button><button className={`memory-toggle ${memoryOpen ? 'active' : ''}`} onClick={() => memoryOpen ? setMemoryOpen(false) : openMemory()} aria-expanded={memoryOpen}><BookOpen size={15} /> <span>Memory</span><span className="memory-number">{storylines.length + proposals.length}</span></button><button className="icon-button mobile-menu" aria-label="Open menu" onClick={() => setMobileNavOpen(true)}><Menu size={18} /></button></div></header>{mode === 'voice' ? <VoiceScreen onBack={() => setMode('text')} onSend={send} onLiveTurn={saveLiveTurn} /> : <><section ref={chatRef} className={`chat-area ${hasMessages ? 'has-messages' : ''}`}>{hasMessages ? <div className="message-list">{messages.map(m => <Message key={m.id} message={m} />)}{busy && <div className="thinking"><RiverMark className="message-river-mark" /><span className="thinking-label">river is thinking</span><i /><i /><i /></div>}</div> : <EmptyState user={user} onMemory={openMemory} onPrompt={send} />}</section>{error && <div className="connection-notice" role="status">{error}</div>}<Composer onSend={send} busy={busy} mode={mode} setMode={setMode} /></>}</main>{memoryOpen && <><button className="memory-backdrop" aria-label="Dismiss memory overlay" onClick={() => setMemoryOpen(false)} /><MemoryPanel storylines={storylines} proposals={proposals} view={memoryView} onUpdate={update} onDelete={remove} onApprove={approveProposal} onReject={rejectProposal} onHistory={memoryHistory} onClose={() => setMemoryOpen(false)} /></>}</div>{mobileNavOpen && <MobileMenuPanel onClose={() => setMobileNavOpen(false)} onNew={newThread} onToday={() => setTodayOpen(true)} onMemory={openMemory} onPrivacy={() => setPrivacyOpen(true)} onLogout={onLogout} />}{todayOpen && <TodayPanel reminders={reminders} storylines={storylines} onClose={() => setTodayOpen(false)} onOpenMemory={() => { setTodayOpen(false); openMemory() }} />}{privacyOpen && <PrivacyPanel user={user} enabled={memoryEnabled} memoryMode={memoryMode} retentionDays={retentionDays} onToggle={toggleMemory} onMemoryModeChange={changeMemoryMode} onRetentionChange={changeRetention} onExport={exportData} onClose={() => setPrivacyOpen(false)} onAccountDeleted={onLogout} />}{searchOpen && <SearchPanel onClose={() => setSearchOpen(false)} onSelectThread={loadThread} />}</>
}
...
function Root() {
  ...
  if (checking) return <div className="loading-screen"><RiverMark className="brand-mark" /><Loader2 className="spin" size={18} /></div>
