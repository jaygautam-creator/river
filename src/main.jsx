...
function PrivacyPanel({ user, enabled, memoryMode, retentionDays, onToggle, onMemoryModeChange, onRetentionChange, onExport, onClose, onAccountDeleted }) {
  ...
  const loadSecurity = async () => { try { const [sessionData, eventData] = await Promise.all([api('/api/auth/sessions'), api('/api/auth/security-events')]); setSessions(sessionData.sessions.filter(session => !session.revoked_at)); setSecurityEvents(eventData.events || []) } catch {} }
...
function VoiceScreen({ onBack, onSend, onLiveTurn }) {
  ...
  const liveSocketRef = useRef(null), liveProcessorRef = useRef(null), liveInputSourceRef = useRef(null), liveSourcesRef = useRef(new Set()), liveNextPlaybackRef = useRef(0), liveInputRef = useRef(''), liveOutputRef = useRef('')
  ...
  const beginLive = async session => {
    const stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true, channelCount: 1 } }); streamRef.current = stream
    const AudioContextClass = window.AudioContext || window.webkitAudioContext; const context = new AudioContextClass(); contextRef.current = context; if (context.state === 'suspended') await context.resume()
    ...
    socket.onerror = () => { if (conversationRef.current) { setVoiceState('error'); setMessage('Live voice connection failed. Try again or use River’s reliable turn-based voice mode.') } }
    socket.onclose = event => { if (conversationRef.current && !event.wasClean) { setVoiceState('error'); setMessage('Live voice disconnected. Try reconnecting; River’s text mode remains available.') } }
  }
  ...
  const begin = async () => {
    ...
    try {
      ...
      if (live?.enabled) { conversationRef.current = true; await beginLive(live); return }
