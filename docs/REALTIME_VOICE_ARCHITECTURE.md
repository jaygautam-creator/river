...
`GET /api/voice/live/session` returns `501` until `REALTIME_VOICE_GATEWAY_URL` points to an independently deployed gateway. Once configured it issues a purpose-limited 60-second gateway token—not the normal River session cookie—so the browser can establish one live connection without receiving a long-lived credential. River's included Cloudflare gateway owns Gemini Live setup, constrains browser messages to real-time input/control events, checks the configured origin, and relays streamed provider audio and transcripts. It is an implemented beta path, not evidence that the required real-world latency and safety validation has been completed.
