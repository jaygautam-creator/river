...
const updatedRetention = await fetch(`${base}/api/privacy/preferences`, { method: 'PUT', headers: { ...authHeaders, 'content-type': 'application/json' }, body: JSON.stringify({ memory_enabled: true, retention_days: 90 }) })
if (!updatedRetention.ok || (await updatedRetention.json()).retention_days !== 90) throw new Error('Privacy retention update failed')
